...
dirs = ['data/VOC2007/imgwithannot/']
classes = ["aeroplane", "bicycle", "bird", "boat", "bottle", "bus", "car", "cat", "chair", "cow", "diningtable", "dog",
           "horse", "motorbike", "person", "pottedplant", "sheep", "sofa", "train", "tvmonitor"]
class_new = ["cat", "dog", "person", "car", "horse"]
    ...
    for obj in root.iter('object'):
        ...
        if cls in class_new:
            list_bbox.append([xmlbox.find('xmin').text, xmlbox.find('xmax').text, xmlbox.find('ymin').text,
                              xmlbox.find('ymax').text, class_new.index(cls)])
    return list_bbox
...
def process_testset(file, output_path, img_path, bboxes):
    ...
    for i in range(len(bbs_aug.bounding_boxes)):
        ...
        obj = [box.x1, box.y1, box.x2, box.y2]
        file.write(",".join([str(int(a)) for a in obj]) + ',' + str(bboxes[i][4]) + ' ')
        file_gt.write(str(class_new[bboxes[i][4]]))
...
def process_trainval():
    for dir_path in dirs:
        ...
        file_train = open(dir_path.replace('VOC2007/imgwithannot/', '') + 'train_annotation.txt', 'w')
        file_val = open(dir_path.replace('VOC2007/imgwithannot/', '') + 'valid_annotation.txt', 'w')
        list_five = []
        ...
        class_names.write("[\'" + "\',\'".join([str(a) for a in class_new]) + "\']")


def process_test():
    dirs = ['data/test/VOC2007/imgwithannot/']
    output_path = 'data/VOC/'
    ...
    for dir_path in dirs:
        image_paths = getImagesInDir(dir_path)
        file_test = open(dir_path.replace('test/VOC2007/imgwithannot/', '') + 'test_annotation.txt', 'w')
        list_five = []
